...
public class Ex3DLista10 {
    ...
    public static void testeB(double quant) {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < nCores; j++) {
                threads[j] = new Thread(new OperacaoSimples(q));
                threads[j].start();
            }
    ...
    public static void testeC(int ini, int fim) {
        ...
        for (int a = ini; a < fim; a++) {
            ...
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < nCores; j++) {
                    threads[j] = new Thread(new OperacaoSimples(quant));
                    threads[j].start();
                }
                for (Thread thread : threads) {
                    try {
